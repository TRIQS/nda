...
/**
 * @file
 * @brief Provides a generic interface to batched versions of the LAPACK/cuSOLVER `orgqr` routine.
 */
...
namespace nda::lapack {
   ...
   * @brief Interface to batched versions of the LAPACK/cuSOLVER `%orgqr` routine.
   *
   * @details This function generates the orthogonal matrix \f$ \mathbf{Q}_i \f$ for each matrix in a batch indexed by
   * \f$ i \in \{ 0, \ldots, N_b - 1 \} \f$ from the elementary reflectors and scalar factors produced by
   * nda::lapack::geqrf_batch. Here, \f$ N_b \f$ is the batch size. See also nda::lapack::orgqr.
   * 
   ...
   * matrices in the batch and calls nda::lapack::orgqr on the first \f$ \min(m, n) \f$ columns of each slice. For wide
   * matrices (\f$ m < n \f$), this produces an \f$ m \times m \f$ orthogonal matrix in the first \f$ m \f$ columns of
   * each slice; the remaining columns are left untouched.
   *
   * @note \f$ \mathbf{A} \f$ and \f$ \mathbf{T} \f$ are required to have nda::F_layout.
   *
   * @tparam A nda::blas_lapack::BlasArrayReal<3> type.
  ...
  int orgqr_batch(A &&a, TAU &&tau, W &&work = vector_value_t<A>{}) { // NOLINT (temporary views are allowed here)
    ...
    int info = 0;
    for (int i = 0; i < n_b; ++i) {
      int local_info = orgqr(a(range::all, range(k), i), tau(range::all, i), work);
