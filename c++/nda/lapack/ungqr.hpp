...
/**
 * @file
 * @brief Provides a generic interface to the LAPACK/cuSOLVER `ungqr` routine.
 */
...
namespace nda::lapack {
  ...
  int ungqr(A &&a, TAU &&tau, W &&work = vector_value_t<A>{}) { // NOLINT (temporary views are allowed here)
    ...
    if constexpr (run_on_device) {
      tmp_lwork = device::ungqr_buffer_size(m, n, k, a.data(), get_ld(a), tau.data());
    } else {
      f77::ungqr(m, n, k, a.data(), get_ld(a), tau.data(), &tmp_lwork, -1, info);
    }
    ...
    if constexpr (run_on_device) {
      device::ungqr(m, n, k, a.data(), get_ld(a), tau.data(), work.data(), lwork, info);
    } else {
      f77::ungqr(m, n, k, a.data(), get_ld(a), tau.data(), work.data(), lwork, info);
    }
