...
namespace nda::lapack {
  ...
  class gelss_worker {
     ...
     * @return A `std::pair<matrix<T>, fp_type>` containing the solution matrix \f$ \mathbf{X} \f$ and the error \f$
     * \epsilon \f$.
     */
     ...
     * @return A `std::pair<vector<T>, fp_type>` containing the solution vector \f$ \mathbf{x} \f$ and the error \f$
     * \epsilon \f$.
     */
   ...
   * \f[
   *   \mathbf{A}_E \mathbf{X} = \begin{bmatrix} \mathbf{A} \\ \mathbf{A}^H \end{bmatrix} \mathbf{X} =
   *   \begin{bmatrix} \mathbf{B} \\ \tilde{\mathbf{B}} \end{bmatrix} = \mathbf{B}_E \; ,
   * \f]
   * where the original problem \f$ \mathbf{A} \mathbf{X} = \mathbf{B} \f$ has been extended by \f$ \mathbf{A}^H
