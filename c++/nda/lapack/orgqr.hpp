...
namespace nda::lapack {
   ...
   * columns. On exit, the \f$ m \times n \f$ matrix \f$ \mathbf{Q} \f$.
  ...
  int orgqr(A &&a, TAU &&tau) { // NOLINT (temporary views are allowed here)
    ...
    double tmp_lwork{};
    int info = 0;
    lapack::f77::orgqr(m, n, k, a.data(), get_ld(a), tau.data(), &tmp_lwork, -1, info);
    int lwork = static_cast<int>(std::ceil(tmp_lwork));

    // allocate work buffer and perform actual library call
    array<double, 1> work(lwork);
    lapack::f77::orgqr(m, n, k, a.data(), get_ld(a), tau.data(), work.data(), lwork, info);
