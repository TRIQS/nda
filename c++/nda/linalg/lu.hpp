...
namespace nda::linalg {
   ...
   *
   * @details If the orginal matrix \f$ \mathbf{A} \f$ is of size \f$ m \times n \f$, then the returned \f$ \mathbf{L}
   ...
   */
  template <typename LP = F_layout>
  auto get_lu_matrices(MemoryMatrix auto const &a) {
    // input, output types and static assertions
    using input_t  = std::remove_cvref_t<decltype(a)>;
    using output_t = matrix<typename input_t::value_type, LP>;
    static_assert(nda::mem::have_host_compatible_addr_space<input_t> and nda::blas::has_F_layout<input_t>);
   ...
   */
  template <typename LP = F_layout>
  auto lu_in_place(MemoryMatrix auto &&a) { // NOLINT (temporary views are allowed here)
    // input, output types and static assertions
    using input_t = std::remove_cvref_t<decltype(a)>;
    static_assert(nda::mem::have_host_compatible_addr_space<input_t> and nda::blas::has_F_layout<input_t> and is_blas_lapack_v<get_value_t<input_t>>);
