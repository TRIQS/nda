...
namespace nda::permutations {
  ...
  template <int Rank> constexpr std::array<int, Rank> decode(uint64_t binary_representation) {
    auto result = nda::make_initialized_array<Rank>(0);
    for (int i = 0; i < Rank; ++i) result[i] = (binary_representation >> (4 * i)) & 0b1111ull;
    return result;
  }

  template <int Rank> constexpr uint64_t encode(std::array<int, Rank> const &permutation) {
    uint64_t result = 0;
