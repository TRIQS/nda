...
namespace nda::tensor {
  ...
  */
  template <Array X, MemoryArray B>
    requires((MemoryArray<X> or nda::blas::is_conj_array_expr<X>) and get_rank<X> == get_rank<B> and have_same_value_type_v<X, B>)
  void assign(get_value_t<X> alpha, X const &x, std::string indxA, B &&b, std::string indxB, [[maybe_unused]] devStream_t const stream = 0) {
    ...
    if constexpr (mem::have_device_compatible_addr_space<A, B>) { // on device
      auto rec_until = [](auto &&A_, auto &&B_) -> void {
        auto rec_until_impl = [](auto &&At, auto &&Bt, auto &impl) -> void {
          if constexpr ((rank == 1)
                        or ((has_layout_strided_1d<decltype(At)> or has_contiguous_layout<decltype(At)>)
                            and (has_layout_strided_1d<decltype(Bt)> or has_contiguous_layout<decltype(Bt)>))) {
            Bt() = At();
          } else {
            long n = Bt.extent(0);
