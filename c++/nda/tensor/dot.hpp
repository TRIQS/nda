...
namespace nda::tensor {
  ...
  template <Array X, Array Y>
    requires((MemoryArray<X> or nda::blas::is_conj_array_expr<X>) and (MemoryArray<Y> or nda::blas::is_conj_array_expr<Y>)
             and have_same_value_type_v<X, Y> and is_blas_lapack_v<get_value_t<X>>)
  get_value_t<X> dot(X const &x, std::string_view const indxX, Y const &y, std::string_view const indxY) {
