...
namespace nda::tensor {
  ...
  void add([[maybe_unused]] get_value_t<X> alpha, X const &x, std::string_view const indxX, [[maybe_unused]] get_value_t<X> beta,
           [[maybe_unused]] B &&b, std::string_view const indxY, [[maybe_unused]] devStream_t const stream = 0) {
  ...
  void add([[maybe_unused]] get_value_t<X> alpha, X const &x, std::string_view const indxX, [[maybe_unused]] get_value_t<Y> beta, Y const &y,
           std::string_view const indxY, [[maybe_unused]] C &&c, std::string_view const indxC, [[maybe_unused]] devStream_t const stream = 0) {
    using nda::blas::is_conj_array_expr;
    auto to_mat = []<typename Z>(Z const &z) -> auto & {
      if constexpr (is_conj_array_expr<Z>)
        return std::get<0>(z.a);
