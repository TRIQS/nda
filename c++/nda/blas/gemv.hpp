...
namespace nda::blas {
  ...
  void gemv(typename A::value_type alpha, A const &a, B const &b, typename A::value_type beta, Out &&c) {
    ...
    if (Ca().extent(1) != Cb().size())
      NDA_RUNTIME_ERROR << "Dimension mismatch in gemv : A : " << Ca().shape() << " while X : " << Cb().shape();
    char trans_a = get_trans(Ca(), false);
    int m1 = get_n_rows(Ca()), m2 = get_n_cols(Ca());
    int lda = get_ld(Ca());
    f77::gemv(&trans_a, m1, m2, alpha, Ca().data_start(), lda, Cb().data_start(), *Cb().indexmap().strides().data(), beta, c.data_start(), *c.indexmap().strides().data());
  }
