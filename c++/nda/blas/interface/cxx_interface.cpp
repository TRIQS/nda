...
#endif
  inline auto *mklcplx(nda::fcomplex *c) { return reinterpret_cast<MKL_Complex8 *>(c); }               // NOLINT
  inline auto *mklcplx(nda::fcomplex const *c) { return reinterpret_cast<const MKL_Complex8 *>(c); }   // NOLINT
  inline auto *mklcplx(nda::fcomplex **c) { return reinterpret_cast<MKL_Complex8 **>(c); }             // NOLINT
  inline auto *mklcplx(nda::fcomplex const **c) { return reinterpret_cast<const MKL_Complex8 **>(c); } // NOLINT
...
extern "C" {
double F77_sdot(FINT, const float *, FINT, const float *, FINT);
...
namespace nda::blas::f77 {
  ...
  inline auto *blacplx(fcomplex *c) { return reinterpret_cast<float *>(c); }                // NOLINT
  inline auto *blacplx(fcomplex const *c) { return reinterpret_cast<const float *>(c); }    // NOLINT
  inline auto **blacplx(fcomplex **c) { return reinterpret_cast<float **>(c); }             // NOLINT
  inline auto **blacplx(fcomplex const **c) { return reinterpret_cast<const float **>(c); } // NOLINT
  ...
  void copy(int N, const fcomplex *x, int incx, fcomplex *Y, int incy) { F77_ccopy(&N, blacplx(x), &incx, blacplx(Y), &incy); }
  ...
  float dot(int M, const float *x, int incx, const float *Y, int incy) {
    ...
    float res = 0.0;
    for (int i = 0; i < M; ++i) res += x[i * incx] * Y[i * incy];
    return res;
  }
...
#endif
    return fcomplex{result.real, result.imag};
  }
  ...
  void gemm_vbatch(char op_a, char op_b, int *M, int *N, int *K, fcomplex alpha, const fcomplex **A, int *LDA, const fcomplex **B, int *LDB,
    ...
    nda::vector<int> group_size(batch_count, 1);
    nda::vector<char> ops_a(batch_count, op_a), ops_b(batch_count, op_b);
    nda::vector<fcomplex> alphas(batch_count, alpha), betas(batch_count, beta);
  ...
  void gemm_batch_strided(char op_a, char op_b, int M, int N, int K, fcomplex alpha, const fcomplex *A, int LDA, int strideA, const fcomplex *B,
                          int LDB, int strideB, fcomplex beta, fcomplex *C, int LDC, int strideC, int batch_count) {
#if defined(NDA_USE_MKL) && INTEL_MKL_VERSION >= 20200002
  ...
  void scal(int M, fcomplex alpha, fcomplex *x, int incx) { F77_cscal(&M, blacplx(&alpha), blacplx(x), &incx); }
