...
namespace h5 {
  ...
  template <typename T>
  void h5_write(group g, std::string const &name, T const &x) REQUIRES(std::is_arithmetic_v<T> or is_complex_v<T>) {
    array_interface::write(g, name, array_interface::h5_array_view_from_scalar(x));
  }

  template <typename T>
  void h5_read(group g, std::string const &name, T &x) REQUIRES(std::is_arithmetic_v<T> or is_complex_v<T>) {
    array_interface::read(g, name, array_interface::h5_array_view_from_scalar(x), array_interface::get_h5_lengths_type(g, name));
  }

  template <typename T>
  void h5_write_attribute(hid_t id, std::string const &name, T const &x) REQUIRES(std::is_arithmetic_v<T> or is_complex_v<T>) {
    array_interface::write_attribute(id, name, array_interface::h5_array_view_from_scalar(x));
  }

  template <typename T>
  void h5_read_attribute(hid_t id, std::string const &name, T &x) REQUIRES(std::is_arithmetic_v<T> or is_complex_v<T>) {
