...
//                3   c   f   f   c
//
void test_sym_grp_matrix_permutation(bool parallel) {
...
// symmetry class.
//
void test_sym_grp_matrix_flip_shift(bool parallel) {
  using idx_t      = std::array<long, 2>;
  using sym_t      = std::tuple<idx_t, nda::operation>;
  using sym_func_t = std::function<sym_t(idx_t const &)>;
...
//
void test_sym_grp_tensor_cyclic_triplet(bool parallel) {
