...
TEST(Clef, Lazy) {
  ...
  EXPECT_EQ(eval(make_expr_terminal(V)[i_], i_ = 0), 14);

  make_expr_terminal(V)[i_] << i_ + 2;
  EXPECT_EQ(V[0], 2);
  EXPECT_EQ(V[1], 3);
  EXPECT_EQ(V[2], 4);

  make_expr_terminal(W)[i_] << i_ + make_expr_terminal(V)[i_];
  ...
  make_expr_terminal(v2)[i_][j_] << (i_ + j_ + 1);
