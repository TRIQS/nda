...
static void pointer_2A(benchmark::State &state) {
  ...
  while (state.KeepRunning()) {
    ...
    for (long i0 = 0; i0 < l0; ++i0)
      for (long i1 = 0; i1 < l1; ++i1) benchmark::DoNotOptimize(p[i0 * a.indexmap().strides()[0] + i1 * a.indexmap().strides()[1]] = 10 * i0 + i1);
  }
}
...
static void pointer_2B(benchmark::State &state) {
  ...
  while (state.KeepRunning()) {
    double *__restrict__ p = &(a(0,0));
    for (long i0 = 0; i0 < l0; ++i0)
      for (long i1 = 0; i1 < l1; ++i1) benchmark::DoNotOptimize(p[i0 * s0 + i1 * s1] = 10 * i0 + i1);
  }
}
